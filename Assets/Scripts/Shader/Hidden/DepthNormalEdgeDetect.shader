Shader "_Custom/DepthNormalEdgeDetect"
{
    Properties
    {
		...
		_SampleDistance("SampleDistance",range(0,5)) = 0
    }
        SubShader
        {
            Tags
        ...
        //像素大小
         ...
         void Outline_float(float2 UV, float OutlineThickness, float DepthSensitivity, float NormalsSensitivity, float ColorSensitivity, float4 OutlineColor, out float4 Out) {
			...
			half2 uvDist = _SampleDistance * _CameraColorTexture_TexelSize.xy;

             //根据像素大小缩放偏移的UV量
             
             uvSamples[0] = UV +(uvDist * float2(-1,0));
             uvSamples[1] = UV +(uvDist * float2(1,0));
             uvSamples[2] = UV +(uvDist * float2(0,-1));
             uvSamples[3] = UV +(uvDist * float2(0,1));
             ...
             for (int i = 0; i < 4; i++) {
                 ...
                 normalSamples[i] = DecodeNormal(SAMPLE_TEXTURE2D(_NormalsTexture, sampler_NormalsTexture, uvSamples[i]));
             ...
             edgeColor = edgeColor > (1 / ColorSensitivity) ? edgeColor : 0;
             ...
             Out =  original+ edge*_OutlineColor;
